...
/**
 * Génère le script d'un Short pour très jeunes enfants : une suite d'"items" (un chiffre,
 * une lettre, une couleur ou une forme à la fois), chacun avec une phrase courte à dire et
 * un mot-clé pour chercher une vraie vidéo de stock (enfants/jouets) en fond. Contenu
 * volontairement simple, répétitif et 100% sûr.
 */
export async function generateKidsScript(topic: KidsTopic): Promise<KidsScriptResult> {
  ...
  return kidsScriptSchema.parse(extractJsonObject<KidsScriptResult>(text));
}
