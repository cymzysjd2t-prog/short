...
import type { KidsItem } from "./kids-script.js";
...
async function synthesizeVoiceElevenLabs(text: string, destPath: string): Promise<void> {
  const res = await fetch(`${ELEVENLABS_API}/text-to-speech/${kidsConfig.elevenlabsVoiceId}`, {
    method: "POST",
    headers: {
      "xi-api-key": kidsConfig.elevenlabsApiKey,
...
async function tryFetchPexelsVideo(query: string, destPath: string): Promise<boolean> {
  try {
    const searchRes = await fetch(
      ...
      { headers: { Authorization: kidsConfig.pexelsApiKey } },
    );
    if (!searchRes.ok) {
...
async function ensureBackgroundSegment(query: string, destPath: string, duration: number): Promise<void> {
  ...
  if (!found) {
    await run("ffmpeg", [
      ...
      "lavfi",
      "-i",
      `color=c=#4ECDC4:s=1080x1920:d=${duration.toFixed(2)}`,
      "-c:v",
      "libx264",
      "-pix_fmt",
      "yuv420p",
...
export async function renderKidsVideo(params: { items: KidsItem[] }): Promise<{ videoPath: string }> {
  const workDir = join(tmpdir(), `kids-render-${randomUUID()}`);
  await mkdir(workDir, { recursive: true });

  const fullNarration = params.items.map((item) => item.line).join(" ");
  const audioPath = join(workDir, "voiceover.mp3");
  await synthesizeVoice(fullNarration, audioPath, workDir);

  const duration = await getAudioDuration(audioPath);
  const totalChars = params.items.reduce((sum, item) => sum + item.line.length, 0) || 1;
  ...
  for (let i = 0; i < params.items.length; i++) {
    ...
    if (!item) continue;
    const segmentDuration = duration * (item.line.length / totalChars);
  ...
  for (let i = 0; i < params.items.length; i++) {
    ...
    const lineFile = join(workDir, `line-${i}.txt`);
    ...
    drawtextParts.push(
      `drawtext=fontfile=${FONT_PATH}:textfile=${labelFile}:fontcolor=white:fontsize=160:` +
        `x=(w-text_w)/2:y=h*0.24:enable='between(t,${start.toFixed(2)},${end.toFixed(2)})'`,
    );
    drawtextParts.push(
      `drawtext=fontfile=${FONT_PATH}:textfile=${lineFile}:fontcolor=white:fontsize=48:` +
        `x=(w-text_w)/2:y=h*0.75:box=1:boxcolor=black@0.4:boxborderw=18:` +
